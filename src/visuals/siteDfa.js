// ============================================================
// siteDfa.js — the left sidebar deterministic finite automaton.
//
// The six page sections are modelled as the states of a DFA:
// each state has a single outgoing transition, the first carries
// the initial arrow, the last is accepting, and a cycle edge
// returns to the start. The active state follows the scroll
// context published by main.js.
// ============================================================
...
export function initSiteDfa() {
  ...
  // ---- states and their single outgoing transitions ----------
  const nodes = [];
  const edges = [];
  ...
  SECTIONS.forEach((sec, i) => {
    ...
    if (i === SECTIONS.length - 1) {
      g.appendChild(el("circle", { r: r - 4, class: "dfa__ring dfa__ring--inner" }));
    }
    const num = el("text", { class: "dfa__num", y: 3.4 });
  ...
  machine.register(() => {
    ...
    cyclePath.classList.toggle("is-active", cycleOn);
    cyclePath.setAttribute("marker-end", cycleOn ? "url(#dfa-arrow-active)" : "url(#dfa-arrow)");
